...
Проект "Домашнее задание 13.2" - это код на Python покрытый тестами (см. раздел "Тесты").
Содержит генераторы и декораторы, а также работу с JSON и API.
Поддерживает импорт из CSV и XLSX файлов.
Добавлен функционал с использованием регулярных выражений.
Собран цельный проект.
...
### модуль [decorators.py](src%2Fdecorators.py):

log -  декоратор для логирования выполнения функции. Если задан `filename`, лог записывается в файл, иначе выводится в консоль.

### модуль [utils.py](src%2Futils.py):

load_transactions - Загружает данные о транзакциях из JSON-файла.

### модуль [external_api.py](src%2Fexternal_api.py):

get_exchange_rate - Функция для получения обменного курса и конвертации суммы

### модуль [get_csv_xls.py](src%2Fget_csv_xls.py):

read_transactions_from_csv - Функция считывает финансовые операции из CSV-файла.

read_transactions_from_excel - Функция считывает финансовые операции из XLSX-файла.

### модуль [regulars.py](src%2Fregulars.py):

process_bank_search - Функция поиска банковских операций по описанию с использованием регулярных выражений.

process_bank_operations - Функция подсчета количества операций по заданным категориям с использованием Counter.
...
Выход функции (в консоль или в файл):

- при успешном завершении:

my_function ок. Результат: 6
...
для модуля [decorators.py](src%2Fdecorators.py): [test_decorators_log.py](tests%2Ftest_decorators_log.py)
